...
class DecoderLayer(nn.Module):
    def __init__(
        ...
        self.selfAttn = SelfAttention(n_embed, n_heads, causal=True)
    ...
    def forward(
        ...
        x = x + self.selfAttn(self.lnSelfAttn(x))
...
class EncoderDecoderLM(nn.Module):
    ...
    def forward(
        ...
        _b, ts = encoder_input_ids.shape
        tt = decoder_input_ids.size(1)
        h = self.encWordEmbedding(encoder_input_ids)
        if self.encPos is not None:
            h = h + self.encPos(torch.arange(ts, device=h.device))[None, :, :]
        for layer in self.encoder_layers:
            # reuse DecoderLayer without cross-attn as encoder block
            h = h + layer.selfAttn(layer.lnSelfAttn(h))
            h = h + layer.mlp(layer.lnMlp(h))
        h = self.encLn(h)
